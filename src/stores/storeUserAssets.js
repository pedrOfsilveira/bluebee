...
export const useStoreUserAssets = defineStore("userAssets", () => {
  ...
  const filterByTime = (assets) => {
    ...
    if (assetsOf4.length > 0) {
      intervalOf4 = setInterval(() => {
        dividend(assetsOf4);
      }, 4000);
    }
    if (assetsOf12.length > 0) {
      intervalOf12 = setInterval(() => {
        dividend(assetsOf12);
      }, 12000);
    }
  };
  ...
  const dividend = async (assets) => {
    let dividendTotal = 0;
    assets.forEach((asset) => {
      let assetDividend = asset.ativos.dividendos
        ? asset.ativos.dividendos * asset.quantidade
        : 0;
  ...
  const loadUserAssets = async () => {
    ...
    if (data) {
      ...
      subscribeAssets();
      filterByTime(assets.value);

      try {
        await storeUserChallenges.verifyChallenge();
      } catch {}
    }
  };
  ...
  const subscribeAssets = () => {
    ...
    userAssetsChannel = supabase
      .channel("userAssets-channel")
      .on(
        "postgres_changes",
        {
          event: "*",
  ...
  const buyAsset = async (buyAssetForm, transactionForm) => {
    ...
    let returnIfCanBuy = await storeAuth.updateBalance(
      transactionForm.valor_total,
    );
    ...
    if (searchData.length >= 1 && returnIfCanBuy) {
      const { error } = await supabase
        .from("perfil_ativos")
        .update({
          quantidade: searchData[0].quantidade + transactionForm.quantidade,
        })
        .eq("perfil_id", storeAuth.userDetails.id)
  ...
  const getAssetIndexByIds = (perfil_id, ativo_id) => {
    let findedAsset = assets.value.findIndex((asset) => {
      return (
        storeAuth.userDetails.id === perfil_id && asset.ativos.id === ativo_id
      );
    });
    return findedAsset;
  };
