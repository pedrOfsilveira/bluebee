  ...
  if (loadedStore.length >= 1) {
    const totals = await Promise.all(
      loadedStore.map(async (asset) => {
        ...
        const middle_price =
          (asset.ativos.valor_max + asset.ativos.valor_min) / 2;
...
const formattedSaldo = computed(() => {
  const saldo =
    storeAuth.userDetails && storeAuth.userDetails.saldo != null
      ? storeAuth.userDetails.saldo
      : 0;
...
const badgeLabel = computed(() => {
  if (
    formattedRealFloat.value === null ||
    flutuacao.value === null ||
    isNaN(flutuacao.value)
  )
    return null;
...
<template>
  <div class="home-top-container">
    <div class="greeting text-h6">
      Olá, {{ storeAuth.userDetails.nome }}! 👋
    </div>
    ...
    <div class="chart-placeholder flex" id="patrimonio-box">
      <div>
        ...
        <div
          v-if="patrimonio !== null"
          class="text-h4 text-weight-bolder balance"
        >
          R$ {{ patrimonio.toFixed(2).replace(".", ",") }}
        </div>
