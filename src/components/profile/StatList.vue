...
const { feedback: quizFeedback, lastFeedback } = storeToRefs(quiz);

const attentionMessages = computed(() => {
  const list = [];
  const fb = quizFeedback.value || lastFeedback.value;
...
const medals = computed(() => {
  const list = Array.isArray(storeUserChallenges.challenges)
    ? storeUserChallenges.challenges
    : [];
  return list
...
<template>
  ...
  <StatSection id="se-cuida">
    ...
    <template v-if="attentionMessages.length">
      <StatMessage
        v-for="(m, i) in attentionMessages"
        :key="'att-' + i"
        category="attention"
      >
        {{ m.text }}
  ...
  <StatSection id="sugestoes">
    ...
    <template v-if="suggestionMessages.length">
      <StatMessage
        v-for="(m, i) in suggestionMessages"
        :key="'sug-' + i"
        category="suggestion"
      >
        {{ m.text }}
    ...
    <template v-else>
      <StatMessage category="suggestion">
        Explore novos conteúdos para desbloquear conquistas e melhorar seu
        perfil.
      </StatMessage>
    </template>
  </StatSection>
